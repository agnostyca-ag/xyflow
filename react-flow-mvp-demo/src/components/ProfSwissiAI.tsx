...
interface ProfSwissiAIProps {
  activeWorkspace: WorkspaceId;
  theme: string;
}

const ProfSwissiAI: React.FC<ProfSwissiAIProps> = ({ activeWorkspace, theme }) => {
  ...
  const [iconPosition, setIconPosition] = useState(() => ({ 
    x: Math.max(50, window.innerWidth - 150), 
    y: 80 
  }));
  ...
  const handleDragStop = (e: any, d: any) => {
    ...
    // Check if we dropped on any widget
    if (dragOverWidget) {
      console.log('Dropped on widget - opening context chat!', dragOverWidget);
      setContextMode(dragOverWidget);
      setTimeout(() => {
        setIsIconMode(false);
        setIsExpanding(false);
      }, 100);
  ...
  const quickActions = {
    ...
    academic: ['Check grades', 'Review schedule', 'Plan semester', 'Track progress'],
  ...
  const professorGreetings = {
    ...
    academic: "Good to see you again, Michel! How are your courses progressing this semester? Any challenges I can help with?",
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="professor-actions">
              <button type="button" className="input-action-btn" title="Add Files">
                <img src="/swissi-chat-plus.svg" alt="Add Files" width="16" height="16" />
              </button>
              <button type="button" className="input-action-btn" title="Swissi Tools">
                <img src="/swissi-chat-tools.svg" alt="Swissi Tools" width="16" height="16" />
              </button>
              <button type="button" className="input-action-btn" title="Dictate">
                <img src="/swissi-chat-microphone.svg" alt="Dictate" width="16" height="16" />
              </button>
              <button type="button" className="input-action-btn" title="Conversation mode">
                <img src="/swissi-chat-talk.svg" alt="Conversation mode" width="16" height="16" />
              </button>
            </div>
          </form>
        </div>
